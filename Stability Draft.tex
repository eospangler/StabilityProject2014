...
\begin{equation}
\frac{d^2U}{dd_{i,t}^2}=(x_i -1) x_i \frac{{d_{i,t}}^{x_i -1}}{E_t g_t} - \left[2P_d\left(d_{i,t} \Biggr|\frac{D_{t-1}}{S_t},\sigma \right)A + P_{dd} \left(d_{i,t} \Biggr|\frac{D_{t-1}}{S_t} ,\sigma \right)Ad_{i,t} \right] 
\end{equation}

\noindent For the FOC and SOC Let:    
\begin{center}
$\underbrace{ x_i \frac{{d_{i,t}}^{x_i -1}}{E_t g_t}}_\textrm{J} - \underbrace{\left[P\left(d_{i,t} \Biggr|\frac{D_{t-1}}{S_t},\sigma \right)A + P_{d} \left(d_{i,t} \Biggr|\frac{D_{t-1}}{S_t} ,\sigma \right)Ad_{i,t} \right]}_\textrm{K}$  	
\end{center}

\noindent and: 

\begin{center}
$\underbrace{(x_i -1) x_i \frac{{d_{i,t}}^{x_i -2}}{E_t g_t}}}_\textrm{H} - \underbrace{\left[2P_d\left(d_{i,t} \Biggr|\frac{D_{t-1}}{S_t},\sigma \right)A + P_{dd} \left(d+{i,t} \Biggr|\frac{D_{t-1}}{S_t} ,\sigma \right)Ad_{i,t} \right]}_\textrm{I}$  
\end{center}

\noindent Then the various scenarios are: 
\begin{enumerate}
\item If $1>x_i>0$ and $K<J$, then $FOC < 0$ and $d_{i,t}=0$. 
\item If $1>x_i>0$ and $K>J$, then $FOC > 0$, $SOC<0$, and $d_{i,t}>0$ 
\item If $x_i>1$ and $K<J$, then $FOC < 0$ and $d_{i,t}=0$. 
\item If $x_i>1$, $K>J$, and $H<I$, then $FOC > 0$, $SOC<0$, and $d_{i,t}>0$. 
\item If $x_i>1$, $K>J$, and $H>I$, then $FOC > 0$, $SOC>0$, and $d_{i,t}\rightarrow \infty$. 
\end{enumerate}

Scenarios 1 through 4 pose very little to be concerned about, providing a nice distribution of dissenters amongst the populace. The $\text{5}^{\text{th}}$, however, requires implementations of constraints that bar the SOC from being positive. To avoid this in simulation, if a random draw of $x_i$ is one such that it makes the $SOC>0$ it will be automatically lowered to the maximal value, $x_{max}$, where $SOC<0$ holds. This constrains should not prove too onerous.       
