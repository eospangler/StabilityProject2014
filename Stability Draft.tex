...
\begin{enumerate}
\item If $x_i<0$, then $FOC < 0$ and $d_{i,t}=0$. 
\item If $x_i>0$ and $H<I$, then $FOC < 0$ and $d_{i,t}=0$. 
\item If $1>x_i>0$ and $H>I$; then $FOC > 0$, $SOC <0$, and $d_{i,t}>0$.  
\item If $x_i>1$, $H>I$, and $J<K$; then $FOC > 0$, $SOC<0$, and $d_{i,t}>0$. 
\item If $x_i>1$, $H>I$, and $J>K$, then $FOC > 0$, $SOC>0$, and $d_{i,t}=d_{i,t}^\star $. 
\end{enumerate}

Scenarios 1 through 4 pose very little to be concerned about, providing a nice distribution of dissenters amongst the populace. The $\text{5}^{\text{th}}$, however, requires a bit more explanation. Given the previously stated income constraint, $M$, and by extension the maximum possible amount of dissent, $d_{i,t}^\star$; during situations in which the $FOC$ and $SOC$ are positive the individual would merely dissent the maximum amount possible.  
