...
\begin{center}
$\underbrace{(x_i -1) x_i \frac{{d_{i,t}}^{x_i -2}}{E g_t}}}_\textrm{J} - 
\underbrace{P\left(\frac{D_{t-1}}{S_t}\Big|\sigma \right)(A-1)Ad_{i,t}^{A-2}}_\textrm{K}$  
\end{center}
...
Throughout this paper we believe we have formed a coherent narrative on the interactions of a government and its citizenry. Our simulations have shown that it is not only the resources of a state that matters, but also how the state allocates resource between security and services that dictates overall stability. A short coming of this paper is that the model only applies to cases of internal strife and does not take into account the role of outside agitators. Additionally, this model presents a homogenous state, but could easily be expanded upon to a state where there is factionalization across the state. However, it must be stated that this is a preliminary model that can significant room of expansion in future research endeavors.

The importance of this work is that it establishes a framework of developing new measures of political stability, one that bridges the gap between the top and bottom viewpoints. Throughout this paper we have discussed the concept of aggregated dissent, $D$, but with only limited discussion of how to actually estimate this critical variable. Thankfully, advances and spread of social media technology make this paper more than a theoretical exercise. The companion piece to this paper will delve deeper into this.   
